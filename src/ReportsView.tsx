...
const ReportsView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      // Obtenemos IDs de presupuestos que ya tienen orden
      ...
      const { data: budgets } = await supabase.from('presupuestos').select('id, estimated_value').eq('status', 'APROBADO');
      const pendingOrders = budgets?.filter(b => !budgetIdsInOrders.includes(b.id)).length || 0;
      const revenue = budgets?.filter(b => !budgetIdsInOrders.includes(b.id)).reduce((acc, b) => acc + Number(b.estimated_value), 0) || 0;
        ...
        <div className="reports-grid">
          ...
          <div className="report-card success">
            ...
            <div className="card-content">
              ...
              <p className="card-value">${stats.projectedRevenue.toLocaleString()}</p>
              <p className="card-label">Total en órdenes pendientes</p>
            </div>
          </div>
        </div>
      )}
